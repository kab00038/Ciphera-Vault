...
In Ciphera, open **Settings → Browser extension** and install the native host. Then follow the displayed path to load the included unpacked extension in a Chromium-based browser. Browser filling requires Ciphera to be running with the vault unlocked and only occurs after an explicit user action.
